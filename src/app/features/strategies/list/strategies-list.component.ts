...
@Component({
  ...
  styleUrl: './strategies-list.component.scss',
})
export class StrategiesListComponent {
  ...
  private static readonly GRID_ID = 'strategies';
  private readonly savedState = loadGridState(StrategiesListComponent.GRID_ID);
  ...
  readonly columns: ColumnConfig[] = [
    { key: 'name', headerKey: 'STRATEGIES.COLUMNS.NAME', sortable: true, sortKey: 'name' },
    { key: 'marketTypeDisplay', headerKey: 'STRATEGIES.COLUMNS.MARKET_TYPE', type: 'badge', translate: true, sortable: true, sortKey: 'marketType' },
    { key: 'isActive', headerKey: 'STRATEGIES.COLUMNS.STATUS' },
    {
      key: 'actions',
      headerKey: 'COMMON.COLUMNS.ACTIONS',
      type: 'action',
      actionsResolver: (row) => (row as unknown as StrategyRow).isActive
        ? ['edit', 'deactivate', 'hard-delete']
        : ['edit', 'restore', 'hard-delete'],
    },
  ];

  readonly mobileColumns = ['name', 'marketTypeDisplay', 'actions'];

  readonly displayRows = computed<StrategyRow[]>(() =>
    this.strategies().map(s => ({
      ...s,
      marketTypeDisplay: `STRATEGIES.MARKET_TYPES.${s.marketType.toUpperCase()}`,
      colorSwatch: s.color ?? '',
    }))
  );
  ...
  constructor() {
    this.loadTrigger$.pipe(
      ...
      next: (response) => {
        this.strategies.set(response.model ?? []);
  ...
  onPageChange(event: PageEvent): void {
    ...
    patchGridState(StrategiesListComponent.GRID_ID, {
  ...
  onSortChange(sort: Sort): void {
    ...
    this.pageIndex.set(0);
    patchGridState(StrategiesListComponent.GRID_ID, { pageIndex: 0 });
    this.load();
  }

  onRowAction(event: RowActionEvent<StrategyRow>): void {
    if (event.action === 'edit') {
      this.router.navigate(['/strategies', event.row.id, 'edit']);
  ...
  onCreate(): void {
    this.router.navigate(['/strategies/create']);
  }

  private confirmSoftDelete(id: string): void {
    const data: ConfirmDialogData = {
      title: this.translate.instant('STRATEGIES.CONFIRM_DELETE.TITLE'),
      message: this.translate.instant('STRATEGIES.CONFIRM_DELETE.MESSAGE'),
      confirmText: this.translate.instant('STRATEGIES.CONFIRM_DELETE.CONFIRM'),
  ...
  private softDelete(id: string): void {
    this.service.softDelete(id).subscribe({
      next: () => {
        this.toastService.success('STRATEGIES.DELETE_SUCCESS');
        this.load();
      },
    });
  }
  ...
  private confirmHardDelete(id: string): void {
    const data: ConfirmDialogData = {
      title: this.translate.instant('STRATEGIES.CONFIRM_HARD_DELETE.TITLE'),
      message: this.translate.instant('STRATEGIES.CONFIRM_HARD_DELETE.MESSAGE'),
      confirmText: this.translate.instant('STRATEGIES.CONFIRM_HARD_DELETE.CONFIRM'),
      cancelText: this.translate.instant('COMMON.ACTIONS.CANCEL'),
      type: 'danger',
    };
    this.dialog.open(ConfirmDialogComponent, { data }).afterClosed().subscribe(confirmed => {
  ...
  private hardDelete(id: string): void {
    this.service.hardDelete(id).subscribe({
      next: () => {
        this.toastService.success('STRATEGIES.HARD_DELETE_SUCCESS');
        this.load();
      },
    });
  }
  ...
  private restore(id: string): void {
    this.service.softDelete(id).subscribe({
      next: () => {
        this.toastService.success('STRATEGIES.RESTORE_SUCCESS');
        this.load();
      },
    });
  }
